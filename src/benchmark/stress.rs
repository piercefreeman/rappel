//! Stress benchmark harness using the new Store/Scheduler architecture.
//!
//! This benchmark stress tests the system with complex workflows that include
//! loops, fan-out, and conditionals.
...
/// Configuration for stress benchmark
#[derive(Debug, Clone)]
...
pub struct StressBenchmarkHarness {
    ...
    _python_env: TempDir,
    _registration_server: StressRegistrationServer,
...
impl StressBenchmarkHarness {
    ...
    pub async fn new(
        ...
        let registration_server = StressRegistrationServer::start(Arc::clone(&store)).await?;
        ...
        let (workflow_id, dag) = store
            .get_workflow_by_name("benchmark.stress")
            .await?
            .ok_or_else(|| anyhow!("Benchmark stress workflow not registered"))?;
...
async fn setup_python_env(grpc_port: u16) -> Result<TempDir> {
    ...
    fs::write(
        temp_dir.path().join("benchmark_stress.py"),
...
struct StressRegistrationServer {
    addr: std::net::SocketAddr,
    handle: JoinHandle<()>,
}

impl StressRegistrationServer {
    async fn start(store: Arc<Store>) -> Result<Self> {
        let listener = TcpListener::bind("127.0.0.1:0").await?;
        let addr = listener.local_addr()?;
        let incoming = TcpListenerStream::new(listener);

        let service = StressWorkflowService { store };
...
impl Drop for StressRegistrationServer {
    fn drop(&mut self) {
        self.handle.abort();
    }
}
...
#[derive(Clone)]
struct StressWorkflowService {
    store: Arc<Store>,
}

#[async_trait]
impl proto::workflow_service_server::WorkflowService for StressWorkflowService {
