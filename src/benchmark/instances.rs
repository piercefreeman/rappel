//! Workflow instances benchmark harness.
//!
//! NOTE: Stubbed out pending migration to new store/scheduler model.

use anyhow::Result;
use std::time::Duration;

use crate::{benchmark::common::BenchmarkSummary, store::Store, worker::PythonWorkerConfig};

#[derive(Debug, Clone)]
pub struct WorkflowBenchmarkConfig {
    pub instance_count: usize,
    pub in_flight: usize,
    pub batch_size: usize,
    pub payload_size: usize,
    pub progress_interval: Option<Duration>,
}

impl Default for WorkflowBenchmarkConfig {
    fn default() -> Self {
        Self {
            instance_count: 100,
            in_flight: 32,
            batch_size: 4,
            payload_size: 1024,
            progress_interval: None,
        }
    }
}

pub struct WorkflowBenchmarkHarness {
    _store: Store,
}

impl WorkflowBenchmarkHarness {
    pub async fn new(
        store: Store,
        _worker_count: usize,
        _worker_config: PythonWorkerConfig,
    ) -> Result<Self> {
        Ok(Self { _store: store })
    }

    pub async fn run(&self, _config: &WorkflowBenchmarkConfig) -> Result<BenchmarkSummary> {
        unimplemented!("Workflow benchmark needs migration to new store/scheduler model")
    }

    pub async fn shutdown(self) -> Result<()> {
        Ok(())
    }

    pub fn actions_per_instance(&self) -> usize {
        0
    }
}
