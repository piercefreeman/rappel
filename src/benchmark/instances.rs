//! Workflow instances benchmark harness using the new Store/Scheduler architecture.
//!
//! This benchmark tests workflow execution throughput by running workflows
//! that exercise multiple actions in sequence.
...
/// Configuration for workflow instances benchmark
#[derive(Debug, Clone)]
...
impl WorkflowBenchmarkHarness {
    ...
    pub async fn new(
        ...
        let registration_server = InstancesRegistrationServer::start(Arc::clone(&store)).await?;
        ...
        let (workflow_id, dag) = store
            .get_workflow_by_name("benchmark.instances")
            .await?
    ...
    pub async fn run(&self, config: &WorkflowBenchmarkConfig) -> Result<BenchmarkSummary> {
        ...
        let workflow_input = encode_workflow_input(config.batch_size, config.payload_size);
...
async fn setup_python_env(grpc_port: u16) -> Result<TempDir> {
    ...
    fs::write(
        temp_dir.path().join("benchmark_instances.py"),
...
fn encode_workflow_input(batch_size: usize, payload_size: usize) -> Vec<u8> {
    let arguments = vec![
...
struct InstancesRegistrationServer {
    addr: std::net::SocketAddr,
    handle: JoinHandle<()>,
}

impl InstancesRegistrationServer {
    async fn start(store: Arc<Store>) -> Result<Self> {
        let listener = TcpListener::bind("127.0.0.1:0").await?;
        let addr = listener.local_addr()?;
        let incoming = TcpListenerStream::new(listener);

        let service = InstancesWorkflowService { store };
...
impl Drop for InstancesRegistrationServer {
    fn drop(&mut self) {
        self.handle.abort();
    }
}
...
#[derive(Clone)]
struct InstancesWorkflowService {
    store: Arc<Store>,
}

#[async_trait]
impl proto::workflow_service_server::WorkflowService for InstancesWorkflowService {
