//! Fan-out benchmark harness using the new Store/Scheduler architecture.
//!
//! This benchmark tests workflow parallelism by running workflows with
...
/// Configuration for fan-out benchmark
#[derive(Debug, Clone)]
...
pub struct FanoutBenchmarkHarness {
    ...
    _python_env: TempDir,
    _registration_server: FanoutRegistrationServer,
...
impl FanoutBenchmarkHarness {
    ...
    pub async fn new(
        ...
        let registration_server = FanoutRegistrationServer::start(Arc::clone(&store)).await?;
        ...
        let (workflow_id, dag) = store
            .get_workflow_by_name("benchmark.fanout")
            .await?
            .ok_or_else(|| anyhow!("Benchmark fanout workflow not registered"))?;
    ...
    pub async fn run(&self, config: &FanoutBenchmarkConfig) -> Result<BenchmarkSummary> {
        ...
        let workflow_input = encode_workflow_input(
        ...
        while completed_count < config.instance_count {
            // Launch instances up to in_flight limit
            while pending_instances.len() < config.in_flight
                && (pending_instances.len() + completed_count) < config.instance_count
            {
                ...
                && last_progress.elapsed() >= interval
            {
                ...
                info!(
                    ...
                    total = config.instance_count,
                    rate = format!("{:.0} workflows/s", rate),
                    "benchmark progress"
                );
            ...
            if pending_instances.is_empty() && completed_count < config.instance_count {
    ...
    /// Get the number of actions per workflow instance
...
async fn setup_python_env(grpc_port: u16) -> Result<TempDir> {
    ...
    fs::write(
        temp_dir.path().join("benchmark_fanout.py"),
        fixtures::BENCHMARK_FANOUT,
    )?;

    // Write entrypoint script that registers the workflow by calling run()
    let entrypoint = r#"
import asyncio
from benchmark_fanout import FanoutBenchmarkWorkflow

if __name__ == "__main__":
    # Running the workflow triggers registration with the gRPC server
    asyncio.run(FanoutBenchmarkWorkflow().run())
"#;
            ...
            value: Some(proto::WorkflowArgumentValue {
                kind: Some(proto::workflow_argument_value::Kind::Primitive(
                    proto::PrimitiveWorkflowArgument {
                        kind: Some(proto::primitive_workflow_argument::Kind::IntValue(
                            fan_out_factor as i64,
                        )),
                    },
                )),
            }),
...
struct FanoutRegistrationServer {
    addr: std::net::SocketAddr,
    handle: JoinHandle<()>,
}

impl FanoutRegistrationServer {
    async fn start(store: Arc<Store>) -> Result<Self> {
        let listener = TcpListener::bind("127.0.0.1:0").await?;
        let addr = listener.local_addr()?;
        let incoming = TcpListenerStream::new(listener);

        let service = FanoutWorkflowService { store };
...
impl Drop for FanoutRegistrationServer {
    fn drop(&mut self) {
        self.handle.abort();
    }
}
...
#[derive(Clone)]
struct FanoutWorkflowService {
    store: Arc<Store>,
}

#[async_trait]
impl proto::workflow_service_server::WorkflowService for FanoutWorkflowService {
