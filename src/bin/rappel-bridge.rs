...
//! - RAPPEL_DATABASE_URL: PostgreSQL connection string (required unless in-memory)
//! - RAPPEL_BRIDGE_GRPC_ADDR: gRPC server bind address (default: 127.0.0.1:24117)
//! - RAPPEL_BRIDGE_IN_MEMORY: enable in-memory execution mode for streaming workflows
    ...
    proto::WorkflowArgumentValue {
        kind: Some(proto::workflow_argument_value::Kind::Basemodel(
            proto::BaseModelWorkflowArgument {
                module: "rappel.workflow_runtime".to_string(),
...
async fn main() -> Result<()> {
    ...
    let store = if in_memory {
        ...
        let dsn = env::var("RAPPEL_DATABASE_URL").context("RAPPEL_DATABASE_URL must be set")?;
    ...
    info!(%grpc_addr, in_memory, "rappel bridge starting");
